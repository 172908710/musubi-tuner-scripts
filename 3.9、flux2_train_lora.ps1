...
$guidance_scale = 1.0                                                             # guidance scale | 引导系数（FLUX.2 dev 通常使用 1.0）
...
$vae_dtype = "float32"                                                          # fp16 | fp32 | bf16, default: float32 for FLUX.2
$fp8_base = $False                                                              # fp8
...
$laungh_script = "flux_2_train_network"
...
python -m accelerate.commands.launch --num_cpu_threads_per_process=8 $launch_args "./musubi-tuner/$laungh_script.py" `
  ...
  --logging_dir="./logs" `
  $ext_args
Assert-NativeCommandSucceeded "Command failed: 3.9、flux2_train_lora.ps1"
